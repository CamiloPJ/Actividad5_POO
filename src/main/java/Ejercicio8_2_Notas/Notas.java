...
public class Notas {
    ...
    double calcularPromedio() {
        ...
        // Obtiene el promedio como la division de la suma de notas sobre el total de notas
        return (suma / listaNotas.length);
    }

    /**
     * Metodo que calcula la desviacion estandar del array de notas
     * @return La desviacion estandar del array de notas
     */
    double calcularDesviacion() {
        double prom = calcularPromedio(); // Invoca el metodo para calcular el promedio
        double suma = 0;
        for(int i = 0; i < listaNotas.length; i++) {
            // Aplica formula para la sumatoria de elementos
            suma += Math.pow(listaNotas[i] - prom, 2 );
        }
        return Math.sqrt(suma / listaNotas.length); // Retorna el calculo final de la desviacion
    }
